...
def main():
    ...
    # Clear set-debug-app flag first so "Waiting for Debugger" dialog is dismissed
    ...
    print("\n✅ DEBUGGER ATTACHED & ARMED AT DataRepository.kt LINE 40.")
    print("👉 Tap any button on the emulator (e.g., 'Process Gold Order')...")
    sys.stdout.flush()
    ...
    if hit:
        ...
        print("===========================================")
        locals_before = call_tool("get_variables", {"session_id": session_id, "thread_id": thread_hit, "scope": "local"})
        print("LOCALS AT BREAKPOINT:", json.dumps(locals_before, indent=2))

        print("\n--- MUTATING LOCAL VARIABLE 'amount' TO 2000.0 ---")
        ...
        print("MUTATION RESULT:", json.dumps(mutate_res, indent=2))

        locals_after = call_tool("get_variables", {"session_id": session_id, "thread_id": thread_hit, "scope": "local"})
        print("LOCALS AFTER MUTATION:", json.dumps(locals_after, indent=2))

        print("\n--- RESUMING EXECUTION WITH MUTATED AMOUNT ($2000.0) ---")
        call_tool("step_execution", {"session_id": session_id, "thread_id": thread_hit, "action": "resume_all"})
        print("✅ Execution resumed! Check the UI on the emulator!")
    else:
