...
def main():
    ...
    print("\n✅ DEBUGGER ARMED. Tap 'Process Gold Order' on the emulator now...")
    sys.stdout.flush()
    ...
    if hit:
        ...
        print("\n--- EVALUATING EXPRESSION: 'amount' ---")
        eval1 = call_tool("evaluate_expression", {"session_id": session_id, "thread_id": thread_hit, "expression": "amount"})
        print("eval('amount') ->", json.dumps(eval1, indent=2))

        print("\n--- EVALUATING EXPRESSION: 'customerType' ---")
        eval2 = call_tool("evaluate_expression", {"session_id": session_id, "thread_id": thread_hit, "expression": "customerType"})
        print("eval('customerType') ->", json.dumps(eval2, indent=2))

        print("\n--- EVALUATING EXPRESSION: '_lastOrderResult' ---")
        eval3 = call_tool("evaluate_expression", {"session_id": session_id, "thread_id": thread_hit, "expression": "_lastOrderResult"})
        print("eval('_lastOrderResult') ->", json.dumps(eval3, indent=2))
